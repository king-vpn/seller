...
sleep 1
wget -q -O xraymode.sh myrid.digital/drg/menu/xraymode.sh && chmod +x xraymode.sh && ./xraymode.sh
sleep 1 
wget -q -O /etc/xray/config.json "myrid.digital/drg/configuration/config.json"
chmod +x /etc/xray/config.json
sleep 1 
rm -f /etc/nginx/conf.d/xray.conf
wget -q -O /etc/nginx/conf.d/xray.conf "myrid.digital/drg/configuration/xray.conf"
...
#LIMIT QUOTA
wget -q -O /usr/local/sbin/quota "myrid.digital/drg/limit/quota.sh"
